...
import org.apache.sis.metadata.sql.MetadataFallbackVerifier;
 ...
 * @module
 */
@DependsOn(ScriptRunnerTest.class)
public final strictfp class MetadataSourceTest extends TestCase {
    ...
    public void testOnPostgreSQL() throws Exception {
        try (TestDatabase db = TestDatabase.createOnPostgreSQL("org/apache/sis/metadata", false)) {
            testAll(db);
        }
    }
    ...
    private static void testAll(final TestDatabase db) throws Exception {
        try (MetadataSource source = new MetadataSource(MetadataStandard.ISO_19115, db.source, "org/apache/sis/metadata", null)) {
            source.install();
