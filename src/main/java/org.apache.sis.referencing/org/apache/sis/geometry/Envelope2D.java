...
public class Envelope2D extends Rectangle2D.Double implements Envelope, Emptiable, Cloneable {
     ...
     * {@linkplain CommonCRS#defaultGeographic() default geographic CRS}.
     ...
     * This is different than the default {@link Double#isEmpty()}
    ...
    public Double[] toRectangles() {
         ...
         */
        final Double[] rect = new Double[1 << Integer.bitCount(isWrapAround)];
        for (int i=0; i<rect.length; i++) {
            rect[i] = new Double(x, y, width, height);
        }
        if ((isWrapAround & 1) != 0) {
            /*
             *  (x+width)   (x)
            ...
            final Double start = rect[0];
            final Double next  = rect[1];
        ...
        if ((isWrapAround & 2) != 0) {
            ...
            final Double start = rect[0];
            final Double next  = rect[isWrapAround - 1];    // == 1 if y is the only wraparound axis, or 2 otherwise.
