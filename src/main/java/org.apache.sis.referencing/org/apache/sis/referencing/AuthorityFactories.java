...
final class AuthorityFactories<T extends AuthorityFactory> extends LazySet<T> {
     ...
     * does not exist. Callers should be prepared to either receive an {@link org.apache.sis.referencing.EPSGFactoryFallback} directly if
        ...
        synchronized (EPSG) {
            ...
            if (factory == null) {
                try {
                    ...
                    log(e, false);
                    factory = org.apache.sis.referencing.EPSGFactoryFallback.INSTANCE;
                }
                EPSG[0] = factory;
            }
            return factory;
     ...
     * this method replaces the {@link EPSGFactory} instance by {@link org.apache.sis.referencing.EPSGFactoryFallback} in order to prevent
    ...
    static GeodeticAuthorityFactory fallback(final UnavailableFactoryException e) throws UnavailableFactoryException {
        ...
        synchronized (EPSG) {
            factory = EPSG[0];
            alreadyDone = (factory == org.apache.sis.referencing.EPSGFactoryFallback.INSTANCE);
    ...
    static boolean failure(final UnavailableFactoryException e) {
        if (!(e.getCause() instanceof SQLTransientException)) {
            ...
            synchronized (EPSG) {
                ...
                if (factory == org.apache.sis.referencing.EPSGFactoryFallback.INSTANCE) {      // May have been set in another thread.
                    return false;
                }
                if (unavailable == factory) {
                    ALL.reload();
                    EPSG[0] = org.apache.sis.referencing.EPSGFactoryFallback.INSTANCE;
                    return false;
                }
            }
        }
     ...
     * or an {@link org.apache.sis.referencing.EPSGFactoryFallback} otherwise.</p>
     *
